...
"""Dataset loading and message formatting.

Seeding is exposed as `set_seed()` rather than executed at import time: importing
a data helper should not mutate global RNG or cuDNN state. Call it explicitly
...
def get_datasets(cfg=None, format_data_flag=True, lazy=False):
    ...
    is the path the published training runs used; the lazy path is covered by
    tests but has not been run through a full training job.
    """
